...
PHP_FUNCTION(rlp_encode)
{
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(tx_data), item) {
		if (Z_TYPE_P(item) == IS_STRING) {
            ...
            encrypted = safe_emalloc(sizeof(uint_least8_t), bytes_len, 205);
...
//zend_string_efree(str);
    ...
    memcpy(s, (const char *)(output_hex+64), 64);

    array_init(return_value);

    add_assoc_stringl(return_value, "r", (const char *)r, 64);
    add_assoc_stringl(return_value, "s", (const char *)s, 64);
    add_assoc_long(return_value, "v", recid);
    add_assoc_stringl(return_value, "signature", (const char *)output_hex, 128);
}


/* {{{ proto string secp256k1_sign2(string message, string private_key) */
